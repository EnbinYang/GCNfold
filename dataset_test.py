...
pp_model_path = os.path.join(base_dir, 'model_save', model_type, 'model_unet_pp_{}.pth'.format(config['best_epoch']))
e2e_model_path = os.path.join(base_dir, 'model_save', model_type, 'model_unet_e2e_{}.pth'.format(config['best_epoch']))
...
contact_net = load_model(model_type, base_dir, net_params, 'score', config['best_epoch'])
...
f1_list, p_list, p_list, mcc_list = all_data_test(contact_net, lag_pp_net, device, test_loader)
...
save_to_csv(data=all_data, file_name='archiveII_test', save_format = 'xls', save_type = 'col')
