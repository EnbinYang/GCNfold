...
def save_model(dataset_name, base_dir, model, epoch):
    ...
    torch.save(model.state_dict(), save_dir + dataset_name + '/model_unet_small_' + str(epoch) + '.pth')
...
def load_model(dataset_name, base_dir, net_params, epoch):
    ...
    PATH = save_dir + dataset_name + '/model_unet_small_' + str(epoch) + '.pth'
...
contact_net = GCNFoldNet_UNet(d=d, L=seq_len, device=device, net_params=net_params)
# contact_net = GCNFoldNet_UNet_small(d=d, L=seq_len, device=device, net_params=net_params)
...
for epoch in range(config['best_epoch'] + 1, epochs):
    ...
    save_model(data_type, base_dir, contact_net, epoch)
